...
{
    return vnn_PostProcessMnist( graph );
}
    ...
    )
{
    ...
    if(vnn_UseImagePreprocessNode())
    {
        return vnn_PreProcessMnist_ImageProcess(graph, inputs, input_num);
    }
    */
    return vnn_PreProcessMnist( graph, inputs, input_num );
}
    ...
    )
{
    ...
    tmsStart = get_perf_count();
    graph = vnn_CreateMnist( data_file_name, NULL,
