...
train_dataset = datasets.MNIST(
    root='/home/jiang/Mnist', train=True, transform=transform, download=True
)  # 本地没有就加上download=True
test_dataset = datasets.MNIST(
    root='/home/jiang/Mnist', train=False, transform=transform, download=True
...
def test():
    ...
    acc = correct / total
    print(
        '[%d / %d]: Accuracy on test set: %.1f %% ' % (epoch + 1, EPOCH, 100 * acc)
    )  # 求测试的准确率，正确数/总数
    return acc
